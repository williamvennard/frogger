...
class BscopeData(InstrumentDataHandler):
    ...
    def post(self,company_nickname="", hardware_name="", instrument_name="",slicename=""):
        ...
        #memcache.set(key, to_save)
        #db.put(to_save)
        #result = db.GqlQuery("SELECT * FROM TestResultsDB WHERE company_nickname =:1 and testplan_name =:2 and instrument_name =:3 and start_tse =:4", company_nickname, bscope_content['testplan_name'], instrument_name, int(bscope_content['start_tse']))
        #for r in result:
        #    if r.Current_slice_count == None:
        #        r.Current_slice_count = 0
        #    r.Current_slice_count += 1
        #    r.put()  
        #db.put(to_save)

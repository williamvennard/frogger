...
class Store(BaseDocumentManager):
  ...
  _INDEX_NAME = searchconfig.STORE_INDEX_NAME
  STORE_NAME = 'store_name'
  STORE_LOCATION = 'store_location'
  STORE_ADDRESS = 'store_address'


class U2000(BaseDocumentManager):
  """Provides helper methods to manage U2000 documents.  All U2000 documents
  built using these methods will include a core set of fields (see the
  _buildCoreU2000Fields method).  We use the given U2000 id (the U2000
  ...
  decoupled-- but using the U2000 id as the doc id allows a document to be
  reindexed given its U2000 info, without having to fetch the
  existing document."""

  _INDEX_NAME = searchconfig.U2000_INDEX_NAME
  ...
  # TO DO - Modify sort options to apply to U2000
  # 
  # _SORT_OPTIONS = [
  ...
  #       [CONFIG_NAME, 'config_name', search.SortExpression(
  #           expression=U2000_NAME,
...
# 'accessor' convenience methods
  ...
  def getStartTse(self):
    """Get the value of the 'start_tse' field of an instrument."""
    return self.getFieldVal(self.START_TSE)

  def getCorrectionFreq(self):
    """Get the value of the 'correction_freq' field of an instrument."""
    return self.getFieldVal(self.CORRECTION_FREQ)

  def getMaxValue(self):
    """Get the value of the 'max_value' field of an instrument."""
    return self.getFieldVal(self.MAX_VALUE)

  def getMinValue(self):
    """Get the value of the 'min_value' field of an instrument."""
    return self.getFieldVal(self.MIN_VALUE)

  def setInstrumentType(self, cat):
    """Set the value of the 'cat' (category) field of an instrument."""
    return self.setFirstField(search.NumberField(name=self.instrument_type, value=inst))

  def getOffset(self):
    """Get the value of the 'offset' field of an instrument."""
    return self.getFieldVal(self.OFFSET)
  ...
  def generateRatingsLinks(cls, query, phash):
    ...
    if not ratings_buckets:
      return None
    rlist = []
    for k in range(searchconfig.RATING_MIN, searchconfig.RATING_MAX+1):
      try:
        v = ratings_buckets[k]
      except KeyError:
        return
  ...
  def _buildCoreProductFields(
      cls, start_tse, hardware_name, data, instrument_type, config_name, testplan_name):
    ...
    fields = [search.TextField(name=cls.hardware_name, value=hardware_name),
              # The 'updated' field is always set to the current date.
              search.DateField(name=cls.UPDATED,
                  value=datetime.datetime.now().date()),
              search.TextField(name=cls.CONFIG_NAME, value=config_name),
              ...
              search.TextField(
                  ...
                  value=re.sub(r'<[^>]*?>', '', testplan_name)),
              search.AtomField(name=cls.INSTRUMENT_TYPE, value=instrument_type),
              search.NumberField(name=cls.START_TSE, value=start_tse),
              search.NumberField(name=cls.DATA, value=DATA)
             ]
    return fields

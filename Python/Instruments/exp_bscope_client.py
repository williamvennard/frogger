...
def bscope_acq(config, s):    
    ...
    if BL_Open(MY_PROBE_FILE,1):
        ...
        while True:
            ...
            #line = line[:-1]
            #print "line=",line
            #print json.dumps({'key':line})
            #what_to_do = json.loads(line)
            what_to_do ={}
            what_to_do['ctrlkey'] = 'start'
            time_bs = time.time()
            if what_to_do['ctrlkey'] == 'start':
                print 'calling BS_Trace ...'
                time_start = time.time()
                print 'wait time', time_start - time_bs 
                BL_Trace(BL_TRACE_FORCED, BL_SYNCHRONOUS)
                tse = dt2ms(datetime.datetime.now())
                DATA = BL_Acquire()
                freq = get_frequency(DATA, MY_RATE)
